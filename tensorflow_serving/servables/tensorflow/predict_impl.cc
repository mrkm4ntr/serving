...
Status SavedModelPredict(const RunOptions& run_options, ServerCore* core,
  ...
  if (untyped_handle.get()->servable().isType<ModelSelector>()) {
    ...
    TF_RETURN_IF_ERROR(handle->GetModelSpec(request.model_spec().signature_name(), &model_spec)); // TODO default case
    TF_RETURN_IF_ERROR(core->GetServableHandle(model_spec, &bundle));
    signature_name = model_spec.signature_name().empty() ? request.model_spec().signature_name() : model_spec.signature_name();
  } else {
